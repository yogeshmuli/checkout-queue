...
- Requires non-shopping checkout customers to provide either item count or basket size before creating a token; when checkout customers mark themselves as still shopping, the frontend requires basket size and disables item-count entry.
...
- Trial admin dashboard now uses a Smart View layout with sticky header tabs for Live, History, and Foresights; Live shows zone/studio queue cards without a date range, History uses a History-only range filter with range-based check-in/completion/cancellation summaries plus collapsible Recharts graphs, and Foresights uses Trial ML metadata when the model is ready.
...
Demo Tools are disabled unless `ENABLE_DEMO_TOOLS=true` and are restricted to `SUPER_ADMIN`. The ML training-data seed endpoint creates one isolated store with `store_number=D-STORE`, one checkout section with three counters, one cashier staff user for each counter, one trial zone with three studios, one trial-zone assistant for that zone, checkout/trial configs and calendars, promotion events, and enough completed/cancelled/no-show checkout and trial tokens to train both ML models. Each seed request also captures one UTC timestamp and derives ten pending Checkout tokens plus ten pending Trial tokens from it, including lane calling times and current availability, so staff consoles have a realistic live queue. Cleanup deletes only that demo store, related demo users, related demo data, demo ML metadata, and demo artifact folders.

The shared `/app` context selector shows a Super Admin-only Demo Tools panel with status, create, recreate, and cleanup actions for the demo ML training dataset. Long-running demo actions show an in-button spinner and action-specific progress label while requests are running.
