...
import { getStoreAnalytics } from '../../../api/analyticsApi.js';
          ...
          analytics.sections.map((section) => (
            <section key={section.section_id} className="overflow-hidden rounded-lg border border-line bg-white">
              ...
              <div className="flex flex-wrap gap-2 border-t border-line p-5">
                <ResourceLink to={`/app/admin/sections?store_id=${analytics.store.id}`} label="Sections" />
                <ResourceLink to={`/app/admin/counters?section_id=${section.section_id}`} label="Counters" />
                <ResourceLink to={`/app/admin/staff?section_id=${section.section_id}`} label="Staff" />
                <ResourceLink to={`/app/admin/queue?section_id=${section.section_id}`} label="Queue" />
              </div>
            </section>
          ))
        ) : (
          ...
          <div className="space-y-2 p-5">
            {topCounters.map((counter) => (
              <Link
                ...
                to={`/app/admin/queue?counter_id=${counter.counter_id}`}
