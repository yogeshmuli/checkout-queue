...
import { getStoreCalendar, updateStoreCalendar } from '../../../../api/checkout/calendarApi.js';
...
export function Calendar() {
  ...
  useEffect(() => {
    async function loadCalendar() {
      ...
      setMessage('');
      try {
        setForm(toForm(await getStoreCalendar(storeId)));
      } catch (error) {
        showApiErrorToast(error);
        setMessage(getErrorMessage(error));
      } finally {
      ...
      <section className="rounded-lg border border-line bg-white p-5">
        ...
        <div className="mt-5 flex gap-2">
          ...
          <button
            type="button"
            disabled={loading || !storeId}
            onClick={() => storeId && getStoreCalendar(storeId).then((calendar) => setForm(toForm(calendar)))}
