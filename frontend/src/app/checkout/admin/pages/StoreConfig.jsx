...
import { getStoreConfig, updateStoreConfig } from '../../../../api/checkout/storeConfigApi.js';
...
export function StoreConfig() {
  ...
  const loadConfig = useCallback(async () => {
    ...
    setMessage('');
    try {
      setForm(toForm(await getStoreConfig(storeId)));
    } catch (error) {
      showApiErrorToast(error);
      setMessage(getErrorMessage(error));
    } finally {
  ...
  function validateForm() {
    ...
    if (!Number.isFinite(perItem) || perItem < 0 || perItem > 60) {
      errors.per_item_service_minutes = 'Per item service minutes must be between 0 and 60.';
    }
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-lg border border-line bg-white p-5">
        <SectionHeader eyebrow="Store queue config" title="Token and service-time rules" />
      ...
      <section className="rounded-lg border border-line bg-white p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wide text-brand-red">Queue behavior</p>
        ...
        <form className="mt-5 grid gap-4 lg:grid-cols-2" onSubmit={submitConfig}>
          ...
          />
          <Field
            label="Per item service minutes"
            value={form.per_item_service_minutes}
            onChange={(value) => setFormField('per_item_service_minutes', value)}
            error={formErrors.per_item_service_minutes}
            type="number"
            min="0"
            step="0.01"
          />
...
function NoStoresCard() {
  return (
    <section className="rounded-lg border border-dashed border-line bg-white p-5">
      ...
      <p className="mt-1 text-sm text-muted">Create a store first, then queue configuration will appear here.</p>
    </section>
  );
}
