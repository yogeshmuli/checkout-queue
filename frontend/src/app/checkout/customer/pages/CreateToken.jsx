...
export function CreateToken() {
  ...
  const selectedStore = useMemo(
    () => stores.find((store) => String(store.id) === String(form.store_id)),
    [form.store_id, stores],
  );
  const selectedSection = useMemo(
    () =>
      selectedStore?.sections?.find(
        (section) => String(section.id) === String(form.section_id),
      ),
  ...
  const storeLabel =
    selectedStore?.name ||
    (form.store_id ? `Store #${form.store_id}` : "Store not selected");
  const sectionLabel = selectedSection
    ...
    : form.section_id
      ...
      : "No section selected";
  const qrStoreLabel = useMemo(
    () => `${storeLabel} · ${sectionLabel}`,
    [sectionLabel, storeLabel],
  );
  ...
  async function submitJoin(event) {
    ...
    if (!form.is_still_shopping && !form.item_count && !form.basket_size) {
      setLoading(false);
      setMessage(
        "Enter item count or select basket size when you are not still shopping.",
      );
      return;
    }
  ...
  function onChangeItemCount(item_count) {
    if (!form.basket_size) {
      setForm({ ...form, item_count });
      return;
    }
    setForm({ ...form, item_count, basket_size: "" });
  }
      ...
      <section className="mx-auto max-w-md">
        <header className="customer-sticky-header glass-panel rounded-xl border border-white/30 bg-brand-red p-4 text-white shadow-soft">
          <div className="flex items-start gap-3">
            ...
            <Link
              to="/"
              className="flex h-12 w-28 shrink-0 items-center justify-center rounded-md border border-white/40 bg-white/95 p-1 shadow-sm"
          ...
          <p className="mt-3 text-sm text-white/95">
            {storesLoading ? "Loading store details..." : qrStoreLabel}
          </p>
        </header>

        <form
          className="mt-5 space-y-4 rounded-lg bg-white p-5 text-ink shadow-soft"
          onSubmit={submitJoin}
        >
          ...
          <div className="grid grid-cols-2 gap-3">
            <DetailTile
              label="Store"
              value={storesLoading ? "Loading..." : storeLabel}
            />
            <DetailTile
              label="Section"
              value={storesLoading ? "Loading..." : sectionLabel}
            />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <Field
          ...
          <Link
            to="/app/checkout/customer/status"
            className="block rounded-lg bg-brand-blush px-3 py-2 text-center text-sm font-medium text-brand-red"
          >
